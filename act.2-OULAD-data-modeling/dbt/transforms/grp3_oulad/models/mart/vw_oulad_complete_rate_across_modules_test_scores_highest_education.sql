...
WITH completion_metrics AS (
    SELECT 
        ...
        educ.highest_education AS highest_educ,  
    ...
    JOIN {{ ref('dim_oulad_highest_education_grp3') }} AS educ
        ON student.highest_educ_id = educ.highest_educ_id 
    JOIN {{ ref('dim_oulad_courses_grp3') }} AS c
        ON fact.course_id = c.course_id
    WHERE educ.highest_education != '?'
    {% if var('module_code', none) is not none %}
        AND c.code_module = '{{ var('module_code') }}'
    {% endif %}
    GROUP BY c.code_module, educ.highest_education, fr.final_result
)
...
SELECT 
    ...
    ROUND(student_count * 100.0 / SUM(student_count) 
          OVER (PARTITION BY module_code, highest_educ), 1) AS percentage
FROM completion_metrics
ORDER BY module_code, highest_educ, final_result
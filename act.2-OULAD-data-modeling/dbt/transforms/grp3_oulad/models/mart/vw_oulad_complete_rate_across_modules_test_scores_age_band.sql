...
WITH completion_metrics AS (
    ...
    JOIN {{ ref('dim_oulad_age_band_grp3') }} AS age
        ON student.age_band_id = age.age_band_id
    JOIN {{ ref('dim_oulad_courses_grp3') }} AS c
        ON fact.course_id = c.course_id
    WHERE age.age_band != '?'
    {% if var('module_code', none) is not none %}
        AND c.code_module IN ({{ var('module_code') }})
    {% endif %}
    GROUP BY c.code_module, age.age_band, fr.final_result
)
...
SELECT 
    ...
    ROUND(student_count * 100.0 / SUM(student_count) 
          OVER (PARTITION BY module_code, age_band), 1) AS percentage
FROM completion_metrics
ORDER BY module_code, age_band, final_result